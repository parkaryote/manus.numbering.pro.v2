...
# 문장 단위 답안 분할 및 번호 기반 입력 (v1.12.0)

## Phase 1: 연습 모드 - 번호 기반 입력 UI
- [x] autoNumbering=1인 문제에서 엔터 기준으로 답안 분할
- [x] 각 번호별 독립 입력 칸 렌더링
- [x] 현재 번호 표시 (1번, 2번, 3번...)
- [x] Enter 키로 다음 번호로 이동 (뷰포트도 함께 이동)
- [x] 번호 건너뛰기 허용 (1번 비워둔 채 2번부터 입력 가능)
- [x] 원하는 번호를 여러 번 반복 연습 가능

## Phase 2: 연습 모드 - 번호 단위 독립 채점
- [x] 번호 단위로 독립 정답 판정
- [x] 부분 입력 정답 처리 금지
- [x] 연습 시간/통계는 실제 입력이 발생한 번호에만 반영

## Phase 3: 시험 모드 - 번호 기반 입력 및 개별 채점
- [x] 번호 순서 입력 강제 금지
- [x] Enter로 자유롭게 번호 간 이동
- [x] 생각나는 것부터 입력 가능
- [x] 제출 시 각 번호 개별 채점
- [x] 한 번호의 오답이 다른 번호 채점에 영향 금지

## Phase 4: 전체 테스트 및 최종 체크포인트
- [x] 모든 기능 통합 테스트
- [x] 번호 기반 입력 정상 작동 확인
- [x] 최종 체크포인트 생성

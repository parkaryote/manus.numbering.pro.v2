...
# 기능 개선 (v2.3.3)
- [x] 표 문제 연습 모드 고급 기능
  - [x] 틀린 글자 실시간 감지: 입력 중 틀린 글자가 있으면 빠른색으로 표시
  - [x] 힌트 보기 버튼: 각 연습 셌에 힌트 버튼 추가
  - [x] 힌트 기능: 버튼 클릭 시 정답 전체 또는 일부 표시
  - [x] 색상 피드백: 정확 (초록색) / 부분 틀림 (빠른색) / 정답 (초록색) 구분
  - [x] 브라우저 테스트 완료 (틀림 글자/힌늸 기능 기능 단스 확인)


# 기능 개선 (v2.4.0) - PDF/PPT 자동 문제 등록
- [ ] PDF/PPT 파일 업로드 및 텍스트 추출 기능
  - [ ] 파일 업로드 UI 추가 (문제 관리 페이지)
  - [ ] PDF 텍스트 추출 (pdfjs 또는 pdf-parse 라이브러리)
  - [ ] PPT 텍스트 추출 (pptx 라이브러리)
  - [ ] 표 구조 보존 추출
  - [ ] 슬라이드/페이지 순서 유지
- [ ] 추출 텍스트를 문제 형식으로 변환
  - [ ] 문단 단위 문제 분할
...
# 기능 개선 (v2.4.0) - Google Cloud Vision API OCR 기반 대량 문제 등록
- [ ] Google Cloud Vision API 통합
  - [ ] Google Cloud 계정 설정 및 인증
  - [ ] Vision API 활성화
  - [ ] 서버 코드에 API 키 추가
  - [ ] PDF 이미지 변환 및 OCR 처리
  - [ ] 텍스트 추출 및 정리
- [ ] 요금제 및 사용량 추적
  - [ ] 데이터베이스 스키마 업데이트 (ocr_usage 테이블)
  - [ ] 월별 사용량 추적 및 리셋
  - [ ] 대단도 알림 기능 (80% 도달 시)
  - [ ] 대단도 실시간 대단보드 표시
- [ ] PDF/PPT 업로드 및 문제 등록 UI
  - [ ] 파일 업로드 및 드래그 드롭
  - [ ] OCR 처리 대기 중 로딩 남실
  - [ ] 추출된 문제 목록 미리보기
  - [ ] 일괄 등록 및 개별 등록 선택
...
# 기능 추가 (v2.5.0) - Google Cloud Vision API OCR
- [x] OCR 서비스 모듈 구현 (server/ocr.ts)
  - [x] Google Cloud Vision API 서비스 계정 인증
  - [x] PDF → 이미지 변환 (pdftoppm)
  - [x] PPT/PPTX → 이미지 변환 (LibreOffice)
  - [x] 이미지 OCR 처리 (DOCUMENT_TEXT_DETECTION)
  - [x] 한글/영문 언어 힌트 설정
- [x] 파일 업로드 라우터 구현 (server/uploadDocument.ts)
  - [x] Multer 파일 업로드 처리
  - [x] 파일 타입 검증 (PDF, PPT, PPTX, JPEG, PNG, WebP, GIF)
  - [x] 파일 크기 제한 (50MB)
  - [x] OCR 결과 반환
- [x] 서버 엔트리포인트에 라우터 등록 (server/_core/index.ts)
- [x] 프론트엔드 DocumentUpload 페이지 구현
  - [x] 드래그 앤 드롭 파일 업로드
  - [x] OCR 진행률 표시
  - [x] 추출된 텍스트 미리보기 (페이지별 그룹화)
  - [x] 문제 선택/해제 기능
  - [x] 과목 선택
  - [x] 일괄 문제 등록
- [x] App.tsx에 라우트 추가 (/upload, /upload/:subjectId)
- [x] DashboardLayout 사이드바에 메뉴 추가
- [x] Questions 페이지에 "PDF/PPT 가져오기" 버튼 추가
- [ ] 브라우저 테스트 및 버그 수정
- [ ] 사용량 추적 및 요금제 시스템 (v2.6.0)


# 기능 추가 (v2.6.0) - OCR 탭 (선택적 보조 기능)
- [x] OCR 탭 UI 구현
  - [x] Questions 페이지 탭에 "OCR" 탭 추가
  - [x] 파일 업로드 (PDF/PPT/이미지) 인터페이스
  - [x] OCR 처리 중 진행률 표시
  - [x] 추출된 텍스트 미리보기
  - [x] 텍스트 수정 가능 (textarea)
  - [x] "저장" 버튼 (텍스트 탭으로 이동 후 문제 등록)
- [x] 고급 옵션 구현
  - [x] "검토 없이 바로 등록" 토글 스위치
  - [x] 활성화 시 OCR 완료 → 즘시 문제 등록
  - [x] 비활성화 시 기본 흐름 (미리보기 → 저장)
- [x] tRPC 프로시져 추가
  - [x] documents.extractText 프로시져
  - [ ] Google Cloud Vision API 호출 (서비스 계정 인증 연동)

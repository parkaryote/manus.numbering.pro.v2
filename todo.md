...
# 표 문제 연습 모드 개선 (v2.1.0)

## 문제점
- 현재 연습 모드가 시험 모드처럼 정답을 숨기고 채점하는 방식
- 연습 모드는 타이핑 연습이어야 함 (텍스트 문제처럼)

## 수정 사항
- [x] TableEditor: '빈칸' 용어를 '연습 셀'로 변경
- [x] 문제 생성 시: 표를 모두 채우고, "연습할 셀" 지정
- [x] Practice.tsx 표 문제 렌더링 수정:
  - [x] 모든 정답을 회색으로 표시
  - [x] 연습할 셀에 입력 필드 + 실시간 타이핑 피드백 (검은색/빨간색)
...
- [x] 데모 라우트를 인증 없이 접근 가능하도록 수정 (App.tsx)

- [x] 비로그인 사용자 접속 시 Sign in 화면 대신 /demo로 자동 리다이렉트
- [x] 데모 페이지에 로그인 유도 버튼 추가 (저장/통계 기능 사용하려면 로그인)

- [ ] 시험 모드: 여러 줄 답안의 부분 정답 인정 안 됨 (줄바꾸기로 구분된 각 줄을 개별 채점해야 함)

# 버그 수정 (v2.1.0)
- [x] 시험 모드 줄 단위 채점 기능 구현
  - [x] 서버 test.evaluate 프로시저에서 줄 단위 비교 로직 추가
  - [x] 각 줄별로 정답/오답 판정 (lineComparisons 배열)
  - [x] 부분 정답 인식 (예: 5줄 중 2줄 맞음 → 40% 정확도)
  - [x] 피드백 메시지에 "n/total 줄 정답 (정확도: x%)" 표시
  - [x] Test.tsx에서 줄 단위 채점 결과 UI 표시
  - [x] 각 줄별로 초록색/빨간색 박스로 정답 여부 표시
  - [x] vitest 테스트 7개 작성 및 통과

# 버그 수정 (v2.1.1)
- [x] 시험 모드 줄 단위 채점 결과가 UI에 표시되지 않는 문제 해결
  - [x] Test.tsx에서 "유사도" 라벨을 "정확도"로 변경
  - [x] 부분 정답 (2/5 줄) 테스트: 40% 정확도 표시 확인
  - [x] 전체 정답 (5/5 줄) 테스트: 100% 정확도 표시 확인
  - [x] 줄 단위 채점 결과 초록색/빨간색 박스로 표시 확인

# 버그 수정 (v2.1.2)
- [ ] 데모 모드에서 줄 단위 채점이 작동하지 않는 문제 해결
  - [ ] 데모 모드에서 채점 결과 재현
  - [ ] 로그인 모드와 데모 모드의 채점 로직 차이 확인
  - [ ] 데모 모드 채점 로직 수정

# 버그 수정 (v2.1.2)
- [x] 데모 모드에서 줄 단위 채점이 작동하지 않는 문제 해결
  - [x] Test.tsx에서 데모 모드 하드코딩된 더미 결과 제거
  - [x] 데모 모드에서도 실제 채점 로직 실행하도록 수정
  - [x] 부분 정답 (2/5 줄) 테스트: 40% 정확도 표시 확인
  - [x] 줄 단위 채점 결과 초록색/빨간색 박스로 표시 확인

# 버그 수정 (v2.1.3)
- [ ] 데모 모드 시험 제출 시 로그인 페이지로 리다이렉트되- [x] 데모 모드에서 줄 단위 채점이 작동하지 않는 문제 해결
...
# 버그 수정 (v2.2.2)
- [x] 연습 완료 후 정확한 fade out 애니메이션 구현
  - [x] 첫 줄: 진한 회색 글씨로 fade out (text-gray-600)
  - [x] 나머지 줄: 연한 회색 줄로 fade out (text-gray-300)
...
# 기능 추가 (v2.3.0)
- [ ] 사용자 계정의 데모 과목 문제를 데모 버전에 동기화
  - [ ] 해부학, 치아 형태학 데모 과목의 문제 조회
  - [ ] 데모 버전 데이터베이스에 문제 등록
  - [ ] 이미지/표 문제 포함 모든 문제 타입 지원
  - [ ] 데모 모드에서 정상 표시 확인

# 기능 추가 (v2.3.0)
- [x] 데모 과목에 임시 문제 2개 추가
  - [x] 해부학 과목: 정상 교합의 조건 문제 추가
  - [x] 치아 형태학 과목: 상악 우측 제1대구치 특징 문제 추가
  - [x] 데모 모드에서 정상 동작 확인

# 기능 추가 (v2.3.1)
- [x] 사용자 계정의 실제 문제를 데모 과목에 복사
  - [x] "머리뼈아래면의 빈칸을 채우시오" 그림 문제 → 해부학 데모 과목 (ID: 480003)
  - [x] "[표 I-3] 상악 치아 크기" 표 문제 → 치아형태학 데모 과목 (ID: 480004)
  - [x] 데모 모드에서 연습 모드 정상 작동 확인

# 기능 개선 (v2.3.2)
- [x] 표 문제 연습 모드 UI 개선
  - [x] 초기 상태: 정답을 회색으로 표시
  - [x] 입력 시작: 첫 글자 입력 시 정답 텍스트 사라짐
  - [x] 입력 중: 사용자 입력만 검정색으로 표시
  - [x] 정답 완성: 정확하게 입력하면 초록색으로 변경
...
# OCR 탭 Questions 페이지 통합 (v2.1.0)
- [x] Questions 페이지에 OCR 탭 추가
- [x] OCRUploader 컴포넌트 임포트 및 통합
- [x] OCR 결과를 질문 필드로 자동 입력
- [x] 텍스트 탭으로 자동 전환 및 사용자 안내


# 데모 모드 버그 수정 (v2.2.0)
- [x] 현재 연습 카운트 증가 시 누적 연습도 함께 증가
- [x] ESC 키로 연습 완료 가능하도록 수정


# 데모 모드 누적 연습 업데이트 버그 수정 (v2.2.1)
- [x] 데모 모드에서 현재 연습 증가 시 누적 연습도 함께 증가
- [x] Demo 페이지에서 누적 연습 수 실시간 반영


# 데모 모드 누적 연습 로컬 스토리지 저장 (v2.2.2)
- [x] Practice에서 데모 모드 누적 연습 수를 로컬 스토리지에 저장
- [x] Demo 페이지에서 로컬 스토리지에서 누적 연습 수 로드


# 데모 모드 현재 연습과 누적 연습 분리 (v2.2.3)
- [x] 현재 연습은 세션 동안만 유지 (페이지 떠날 때까지)
- [x] 페이지 떠날 때 현재 연습을 누적 연습으로 전환
- [x] 누적 연습만 로컬 스토리지에 저장

# 데모 모드 현재 연습 초기화 (v2.2.4)
- [x] 현재 연습은 세션 동안만 유지되고 페이지 떠날 때 0으로 초기화


# Practice 페이지에 누적 연습 표시 (v2.2.5)
- [x] Practice 페이지의 헤더에 "현재 연습: X / 누적 연습: Y" 표시
- [x] 로컬 스토리지에서 누적 연습 수 읽어서 표시


# 연습 완료 시 누적 연습 동시 증가 (v2.2.6)
- [x] handleCorrectAnswer에서 현재 연습 증가 시 누적 연습도 동시 증가
- [x] handleComplete에서 현재 연습 증가 시 누적 연습도 동시 증가
- [x] 로컬 스토리지에 실시간으로 누적 연습 저장


# 데모 모드 누적 연습 중복 증가 방지 (v2.2.7)
- [x] handleGoBack에서 현재 연습을 누적 연습에 더하는 로직 제거
- [x] 연습 완료 시에만 누적 연습 증가, 돌아가기 시에는 증가 안 함


# 표 문제 연습 카운트 수정 (v2.3.0)
...
# 텍스트 문제 입력 포커스 버그 수정 (v2.9.0)
- [x] 문제 입력 완료 후 포커스가 첫째 줄로 돌아가도록 수정 (questionTextareaRef 추가)
- [x] 입력 폼 리셋 시 포커스 위치 확인 (setSelectionRange(0, 0) 추가)
- [x] 포커스 버그 테스트 (데모 모드에서 문제 생성 후 포커스 동작 확인)

# 연습 모드 포커스 버그 수정 (v2.10.0)
- [x] 연습 완료 후 다음 문제 로드 시 입력창 포커스가 첫 줄로 자동 이동 (setSelectionRange(0, 0) 추가)
- [x] 포커스 스타일(진회색) 자동 표시
- [x] 사용자가 글자 입력 없이도 포커스 시각화

# 연습 모드 포커스 및 초기화 버그 수정 (v2.11.0)
...
# UI 개선 (v2.0.2) - 데모 관리 페이지
- [x] 문제 수 초기 표시 버그 수정 - 0 대신 실제 개수로 표시
- [x] 텍스트 정답 개행 처리 - 엔터로 나뉜 문장을 줄바꿈으로 표시 (whitespace-pre-wrap)
- [x] 이미지 문제 정답 표시 - imageLabels에서 정답 추출하여 [1], [2], [3] 형식으로 표시
- [x] 표 문제 정답 표시 - 구조화된 형식으로 정답 표시
...
# UI 개선 (v2.0.5) - 수정 다이얼로그 문제 유형별 구현
- [x] 기존 문제 등록/수정 컴포넌트 코드 확인
- [x] 텍스트 문제 수정: 문제 + 정답 텍스트 편집

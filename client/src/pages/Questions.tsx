...
export default function Questions({ subjectId }: QuestionsProps) {
  ...
  const [ocrAutoRegister, setOcrAutoRegister] = useState(false);
    ...
    if (activeTab === "ocr") {
      ...
      if (ocrAutoRegister) {
        ...
        createMutation.mutate({
          subjectId,
          question: "OCR 추출 문제",
          answer: ocrExtractedText,
          difficulty: formData.difficulty,
          autoNumbering: formData.autoNumbering ? 1 : 0,
        } as any);
      } else {
        // 기본 모드: 텍스트 탭으로 이동하여 수정 후 저장
        setFormData((prev) => ({
          ...prev,
          answer: ocrExtractedText,
        }));
        setActiveTab("text");
        toast.success("텍스트 탭에서 수정 후 저장하세요");
      }
      return;
    }
  ...
  }, [activeTab, formData, subjectId, createMutation, ocrExtractedText, ocrAutoRegister]);
    ...
    <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "text" | "image" | "table" | "ocr")} className="w-full">
      <TabsList className="grid w-full grid-cols-4">
        ...
        <TabsTrigger value="ocr" className="gap-1">
          <FileUp className="h-3.5 w-3.5" />OCR
        </TabsTrigger>
      </TabsList>
